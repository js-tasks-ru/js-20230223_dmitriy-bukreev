...
export default class ProductForm {
  ...
  async save() {
    ...
    const options = {
      ...
      body: JSON.stringify(product, (key, value) => {
        // Надо по заданию, но в примере этого почему-то нет
  ...
  async render() {
    ...
    let product = this.productDefaults;
    if (this.productId.length) product = await this.loadProduct();
    ...
    this.subElements["sortable-list-container"].append(
  ...
  initEventListeners() {
    this.imageUploader.element.addEventListener(
      "image-uploaded",
